(function ($) {
  ...
  $(function () {
    ...
    if (
      String(window.location.href).search('filter') !== -1 &&
      $('.b-catalog-filter').length &&
      !$('.b-catalog-section').data('landingpage')
    ) {
    ...
    $('.bx-filter-parameters-box-title').click(function (e) {
      e.preventDefault();
      $(this)
        .closest('.bx-filter-parameters-box')
        .find('.bx-filter-block ')
        .slideToggle();
    });
    ...
    $('#priceSliderRange').slider({
      ...
      values: [
        $('#priceSliderRange').data('fact-min-value'),
        $('#priceSliderRange').data('fact-max-value'),
      ],
      step: 1,
      slide: function (event, ui) {
        $('#priceSliderRangeText').text(
          '\u041E\u0442 ' +
            String(ui.values[0]).replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ') +
            ' - \u0434\u043E ' +
    ...
    $('#priceSliderRangeText').text(
      '\u041E\u0442 ' +
        String($('#priceSliderRange').slider('values', 0)).replace(
          /(\d)(?=(\d\d\d)+([^\d]|$))/g,
          '$1 '
        ) +
        ' - \u0434\u043E ' +
        String($('#priceSliderRange').slider('values', 1)).replace(
          /(\d)(?=(\d\d\d)+([^\d]|$))/g,
          '$1 '
        )
    );
